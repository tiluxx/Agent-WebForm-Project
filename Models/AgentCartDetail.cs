...
namespace Agent_WebForm_Project.Models
{
    ...
    public partial class AgentCartDetail
    {
        public string CartID { get; set; }
        public string ProductID { get; set; }
        public Nullable<int> Quantity { get; set; }
    
        public virtual AgentCart AgentCart { get; set; }
        ...
        public List<Product> SelectProductCartQuery(string cartId)
        {
            using (SqlConnection conn = new SqlConnection(ConfigurationManager.ConnectionStrings["DBConn"].ToString()))
            {
                ...
                string sql = "select P.* from Product P where P.ProductID in" +
                    " (select cd.ProductID from AgentCart c, AgentCartDetail cd where c.CartID = cd.CartID and c.CartID = '" + cartId + "')";
        ...
        public void AddCartDetailQuery(string cartID, string productId)
        {
            ...
            {
                conn.Open();
                string sql = "insert into AgentCartDetail values ('" + cartID + "', '" + productId + "', " + 0 + ")";
                SqlCommand cmd = new SqlCommand(sql, conn);
                cmd.ExecuteNonQuery();
            }
        }
        ...
        public void DeleteProductCartQuery(string cartId, string productId)
        {
            ...
            {
                conn.Open();
                string sql = "delete from AgentCartDetail where CartID = '" + cartId + "' and ProductID = '" + productId + "'";
                SqlCommand cmd = new SqlCommand(sql, conn);
                cmd.ExecuteNonQuery();
                conn.Close();
            }
        ...
        public bool CheckProductExist(string cartId, string productId)
        {
            ...
            {
                conn.Open();
                string sql = "select * from AgentCartDetail where CartID = '" + cartId + "' and ProductID = '" + productId + "'";
                SqlCommand cmd = new SqlCommand(sql, conn);
